...
use shade_protocol::{adapter_template, snip20::Snip20Asset};
...
pub static ASSET_LIST: &[u8] = b"assets";

pub fn config_w<S: Storage>(storage: &mut S) -> Singleton<S, adapter_template::Config> {
    singleton(storage, CONFIG_KEY)
}

pub fn config_r<S: Storage>(storage: &S) -> ReadonlySingleton<S, adapter_template::Config> {
